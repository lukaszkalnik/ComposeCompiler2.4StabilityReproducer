...
// Mirror the real SDK: the collected state is a SEALED BASE type (`CartScreenState`) with (at
// least) two subclasses. The UI collects the base type and `when`-dispatches.
...
// the bug STILL reproduces — so switching to ImmutableList is NOT a fix on its own; the @Immutable
// holder is still wrongly skipped.
@Immutable
